...
function Chat() {
  ...
  function updateChannelsList(selectFirstChannel: boolean) {
    ...
    Promise.all([fetchPublicChannels(), fetchUserProtectedChannels(user!.id), fetchUserPrivateChannels(user!.id), fetchUserDMs(user!.id)])
      .then((channels) => {
        try {
          const publicChannels: Channel[] = channels[0];
          const protectedChannels: Channel[] = channels[1];
          const privateChannels: Channel[] = channels[2];
          const userDMs: Channel[] = channels[3];
          const allChannelsDB = [...publicChannels, ...protectedChannels, ...privateChannels, ...userDMs];
          setAllChannels(allChannelsDB);

          const newSelectedChannel = selectFirstChannel
            ? allChannelsDB[0]
            : allChannelsDB.find(({ id }) => id === selectedChannel?.id) || allChannelsDB[0];
        ...
        try {
          ...
          if (user) {
            ...
            const canWriteList = membersList.filter((userNickname: string) => {
              if (mutedList.indexOf(userNickname) !== -1) return false;
              return true;
            });
  ...
  function createChannel(newChannelValue: string) {
    ...
    return new Promise((resolve, reject) => {
      ...
      return chatSocket!.emit("createChannel", { channelName: newChannelValue, token }, (response: ChatResponse) => {
        if (!response.ok) {
          if (response.status === 409) return reject(response);
          return reject(false);
        }
        return resolve(true);
      });
  ...
  function onInviteGame(data: any) {
    ...
    try {
      if (data.invited === user!.nickname) {
        ...
        setGameInvitations((prevInvitations) => [...prevInvitations, { gameId: data.gameId, dmName: data.dmName }]);
      }
    } catch (error) {
      throwAsyncError(error);
    }
        ...
        .then((requests) => {
          if (requests.length !== 0) {
            const pendingFriendRequests = requests.map((request: any) => {
              if (request.user1 === user!.nickname) return request.user2;
              else return request.user1;
            });
    ...
    function onDeletedChannel(oldChannel: Channel) {
      ...
      try {
        ...
        const newChannelsList = allChannels.filter((channel: Channel) => {
          if (channel.name === oldChannel.name) return false;
          return true;
        });
  ...
  const handleSelectedChannel = (channelDetails: Channel) => {
    ...
    try {
      ...
      const tmp = channelsWithNewMsg.filter((name) => {
        if (name === channelDetails.name) return false;
        return true;
      });
  ...
  const handleDMrequest = (player: string) => {
    ...
    try {
      ...
      chatSocket!.emit("createDm", { player }, (response: ChatResponse) => {
        if (!response.ok) throwAsyncError("Error sending DM request " + response.status + " " + response.statusText);
      });
    } catch (error) {
      throwAsyncError(error);
    }
  ...
  return (
    ...
    >
      ...
      <div className="Chat__header">
        <Typography className="Chat__header__title" sx={{ color: "#015958", fontWeight: "bold", fontSize: "13px" }}>
          Channels
        </Typography>
      ...
      <div className="Chat__main">
        ...
        <div className="Chat__channels">
          <form className="Chat__newChannelForm" onSubmit={handleSubmitNewChannel}>
            <input ref={newChannel} className="Chat__newChannelForm__input inputChat" type="text" placeholder="New channel" />
          </form>
          <div className="ChannelsList">
            {allChannels.map((channelListItem) => (
              <SingleChannel
                key={channelListItem.id}
        ...
        {canSee && (selectedChannel.type !== "PROTECTED" || (selectedChannel.type === "PROTECTED" && isMember)) && (
          <Live
            ...
            friendRequest={friendRequest.filter((request) => {
              if (request === selectedChannel.label && !isBlocked(selectedChannel.label)) return true;
              return false;
            })}
        ...
        {canSee && selectedChannel.type === "PROTECTED" && !isMember && <LiveRestrictedArea channel={selectedChannel} />}
        {!canSee && <LiveForbidden />}
      </div>

      {/* Channel details */}
      {selectedChannel.name === "ALL" && (
        <ChannelDetailsALL channel={selectedChannel} channelAdmins={channelAdmins} channelMembers={channelMembers} channelInvited={channelInvited} />
      )}
      {readOnly && (
      ...
      {!readOnly &&
        selectedChannel.name !== "ALL" &&
        (selectedChannel.type !== "PROTECTED" || (selectedChannel.type === "PROTECTED" && isMember)) &&
      ...
      {!readOnly && selectedChannel.type === "DM" && (
        <ChannelDetailsDM channel={selectedChannel} channelAdmins={channelAdmins} channelMembers={channelMembers} channelInvited={channelInvited} />
      )}
