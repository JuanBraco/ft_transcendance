...
export default function ChatPasswordDialog({ displayChatPassword, setDisplayChatPassword, channel }: Props) {
  /*
   ** ********************************************************************************
   ** VARIABLES
   ** ********************************************************************************
  ...
  const handlePasswordSubmit = (event: React.FormEvent) => {
    ...
    try {
      ...
      chatSocket!.emit("joinChannel", { channelName: channel.name, password: password }, (response: ChatResponse) => {
        if (!response.ok) {
          ...
          } else throwAsyncError("Error joinChannel channel " + response.status + " " + response.statusText);
        }
      });
    } catch (error) {
        ...
        <DialogContent>
          <Box sx={{ minWidth: 120 }}>
            <FormControl fullWidth>
              <input id="password" ref={pwdRef} type="password" placeholder="Enter password" onChange={updatePassword} />
            </FormControl>
          </Box>
          <DialogActions>
            <Button onClick={handleCancel}>Cancel</Button>
            <Button onClick={handlePasswordSubmit} disabled={isPasswordEmpty()}>
              OK
            </Button>
          </DialogActions>
        </DialogContent>
